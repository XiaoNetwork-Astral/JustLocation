...
function run(args, input) {
  ...
  if (result.status !== 0)
    throw new Error(
      `adb ${args[0]} failed (${result.status}):\n${result.stdout}\n${result.stderr}`,
    );
...
try {
  ...
  const frames = [
    { version: 1, op: 'status' },
    {
      version: 1,
      op: 'start',
      config: { position, scope: { mode: 'apps', packages: ['me.idk.justlocation.probe'] } },
    },
  ...
  const replies = run(
    ['shell', `${remote}/justlocationd stdio`],
  ...
  const output = run([
    'shell',
    `CLASSPATH=${remote}/probe.zip LD_LIBRARY_PATH=${remote} timeout 45 app_process /system/bin --nice-name=justlocation-probe me.idk.justlocation.bridge.RuntimeProbe ${remote}`,
  ]);
  console.log(output);
  assert.ok(output.includes('JUSTLOCATION_PROBE_OK'));
  const bridge = run([
    'shell',
    `env -u LD_LIBRARY_PATH CLASSPATH=${remote}/probe.zip timeout 45 app_process /system/bin --nice-name=justlocation-probe me.idk.justlocation.bridge.RuntimeProbe ${remote} bridge`,
  ]);
  console.log(bridge);
  ...
  run([
    'shell',
    `rm -f ${files.map(([, name]) => `${remote}/${name}`).join(' ')}; rmdir ${remote}`,
  ]);
  assert.equal(
    run(['shell', 'pidof system_server']),
    before,
    'system_server PID changed during probe',
  );
