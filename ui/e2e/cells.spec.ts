...
test('mobile cell panel configures detected cards, applies target data and preserves settings when toggled', async ({
  page,
}) => {
  ...
  await page.addInitScript(() => {
    ...
    const state: any = {
      ...
      cell_hook_ready: true,
      detected_subscriptions: [card],
    ...
    const dataset = {
      ...
      region: {
        ...
        cells: [
          {
            identity: { radio: 'lte', mcc: '460', mnc: '001', tac: 10, ci: 100 },
      ...
      exec: (command: string, _options: string, callback: string) => {
        const frame = JSON.parse(
          new TextDecoder().decode(
            Uint8Array.from(atob(command.split(' ').at(-1)!), (c) => c.charCodeAt(0)),
          ),
        );
        ...
        if (command.includes(' cells ')) {
          host[callback](
            ...
            JSON.stringify({
              version: 1,
              ok: true,
              ...(frame.op === 'query' ? { dataset, cached: true, stale: false } : { settings }),
            }),
            '',
          );
          return;
  ...
  await toggleSwitch(
    page,
    page.getByRole('checkbox', { name: '模拟 SIM 运营商', exact: true }),
    true,
  );
    ...
    await page.evaluate(
      () =>
        (window as any).frames
          .filter((f: any) => f.op === 'set_telephony' && f.config.subscriptions.length)
          .at(-1).config.subscriptions[0].mnc,
    ),
  ).toBe('001');
  await page.getByText('修改运营商', { exact: true }).click();
  ...
  expect(
    await page.evaluate(() => (window as any).frames.some((f: any) => f.op === 'set_cell_region')),
  ).toBe(false);
  ...
  await toggleSwitch(
    page,
    page.getByRole('checkbox', { name: '启用基站模拟', exact: true }),
    false,
  );
  const last = await page.evaluate(() =>
    (window as any).frames.filter((f: any) => f.op === 'set_telephony').at(-1),
  );
