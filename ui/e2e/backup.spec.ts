...
test('imports and downloads a backup, restores visible libraries and exports GPX on a narrow screen', async ({
  page,
}) => {
  ...
  expect(await readFile((await (await gpxEvent).path())!, 'utf8')).toContain(
    '<name>小桥 &amp; 河边</name>',
  );
