...
it('uses detected slot IDs and preserves three-digit MNC when enabling cell simulation', async () => {
  ...
  expect(save).toHaveBeenCalledWith({
    ...
    sim_enabled: false,
    radius_m: 500,
    subscriptions: [{ ...card, enabled: true }],
  });
...
it('keeps unsaved edits through status polling and a failed save', async () => {
  ...
  rerender(
    <TelephonySettings
      state={{ ...state, location_hook_ready: true }}
      busy={false}
      onSave={save}
    />,
  );
...
it('distinguishes missing phone connection from no inserted SIM and refuses to save after a card changes', async () => {
  ...
  const { rerender } = render(
    <TelephonySettings
      state={{ ...state, phone_connected: false, detected_subscriptions: null }}
      busy={false}
      onSave={save}
    />,
  );
  ...
  rerender(
    <TelephonySettings
      state={{ ...state, detected_subscriptions: [] }}
      busy={false}
      onSave={save}
    />,
  );
  ...
  rerender(
    <TelephonySettings
      state={{ ...state, detected_subscriptions: [{ ...card, id: 8 }] }}
      busy={false}
      onSave={save}
    />,
  );
