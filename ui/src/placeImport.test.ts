...
it('honours labels and accepts them in either order', () => {
  expect(parseCoordinateText('纬度: 31.2, 经度: 121.5')).toEqual({
    latitude: 31.2,
    longitude: 121.5,
  });
  ...
  expect(parseCoordinateText('longitude: 121.5, latitude: 31.2')).toEqual({
    latitude: 31.2,
    longitude: 121.5,
  });
...
it('reads the shared shapes of the four map providers', () => {
  expect(parseMapLink('https://uri.amap.com/marker?position=116.404,39.915&name=x')).toEqual({
    ...
    position: { latitude: 39.915, longitude: 116.404 },
  });
  expect(parseMapLink('https://www.amap.com/?lng=116.404&lat=39.915')).toMatchObject({
    provider: 'amap',
    coordinateSystem: 'gcj02',
  });
  expect(parseMapLink('https://map.qq.com/?what=marker&marker=coord:39.915,116.404')).toMatchObject(
    { provider: 'tencent', coordinateSystem: 'gcj02' },
  );
  expect(parseMapLink('https://map.baidu.com/?latlng=39.915,116.404&title=x')).toMatchObject({
    provider: 'baidu',
    coordinateSystem: 'bd09',
  });
  expect(parseMapLink('https://www.google.com/maps/place/x/@39.915,116.404,15z')).toMatchObject({
    provider: 'google',
...
it('never mistakes projected map coordinates for latitude and longitude', () => {
  ...
  expect(
    parseMapLink('https://map.baidu.com/@12947052.9,4825924.6,19z?latlng=39.915,116.404'),
  ).toMatchObject({ provider: 'baidu' });
...
it('routes text the way the spec describes and explains what it cannot read', () => {
  expect(parseImportText('https://map.baidu.com/?latlng=39.915,116.404')).toMatchObject({
    kind: 'link',
  });
  expect(parseImportText('31.2, 121.5')).toEqual({
    kind: 'coordinate',
    position: { latitude: 31.2, longitude: 121.5 },
  });
