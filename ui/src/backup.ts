...
function position(value: unknown): Position {
  ...
  const result = {
    latitude: finite(p.latitude),
    longitude: finite(p.longitude),
    altitude: finite(p.altitude),
    accuracy: finite(p.accuracy),
    speed: finite(p.speed),
    bearing: finite(p.bearing),
  };
  if (
...
function route(value: unknown): SavedRoute {
  ...
  const plan: RoutePlan = {
    points: array(p.points, position),
    speed: finite(p.speed),
  ...
  if (plan.repeat_delay !== undefined && (plan.repeat_delay < 0 || plan.repeat_delay > 86400))
    throw new Error('备份中的路线间隔有误');
  return { id: title(r.id), name: title(r.name), plan };
}
export function parseBackup(text: string): Backup {
  if (new TextEncoder().encode(text).length > maxFileSize)
    throw new Error('请选择 2 MB 以内的备份文件');
  let value;
  try {
  ...
  if (value.format !== 'justlocation' || value.version !== 1)
    throw new Error('不支持这个备份格式，请检查文件或更新模块');
  return {
    format: 'justlocation',
    version: 1,
    places: array(value.places, place),
    routes: array(value.routes, route),
  };
}
export function createBackup(storage: Storage): string {
  const value = {
    format: 'justlocation',
    version: 1,
    places: JSON.parse(storage.getItem(placesKey) || '[]'),
    routes: JSON.parse(storage.getItem(routesKey) || '[]'),
  };
  const text = JSON.stringify(parseBackup(JSON.stringify(value)), null, 2);
  if (new TextEncoder().encode(text).length > maxFileSize)
    throw new Error('数据超过 2 MB，请先分开导出路线');
  return text;
}
...
export function importBackup(backup: Backup, storage: Storage): { places: number; routes: number } {
  ...
  const places = merge(current.places, incoming.places),
    routes = merge(current.routes, incoming.routes);
  const oldPlaces = storage.getItem(placesKey);
  let wrotePlaces = false;
  try {
    storage.setItem(placesKey, JSON.stringify(places));
    wrotePlaces = true;
    storage.setItem(routesKey, JSON.stringify(routes));
  } catch {
    if (wrotePlaces) {
      try {
        if (oldPlaces === null) storage.removeItem(placesKey);
        else storage.setItem(placesKey, oldPlaces);
      } catch {
        throw new Error('保存失败，部分位置可能已导入，请检查列表后重试');
      }
    }
    throw new Error('保存失败，请检查面板的存储空间');
  }
  return {
    places: places.length - current.places.length,
    routes: routes.length - current.routes.length,
  };
}
