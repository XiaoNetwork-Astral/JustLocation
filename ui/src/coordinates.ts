...
function toGcj(p: Point): Point {
  ...
  return {
    latitude: p.latitude + (lat * 180) / (((axis * (1 - eccentricity)) / (magic * root)) * pi),
...
function fromGcj(p: Point): Point {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const latError = projected.latitude - p.latitude,
      lonError = projected.longitude - p.longitude;
...
export function parsePosition(latitude: string, longitude: string, altitude: string): Position {
  if (
    [latitude, longitude, altitude].some(
  ...
  if (Math.abs(lat) > 90 || Math.abs(lon) > 180)
    throw new Error('纬度须在 −90～90，经度须在 −180～180');
  return {
    latitude: lat,
    longitude: lon,
