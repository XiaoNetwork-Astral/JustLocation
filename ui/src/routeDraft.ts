import { parsePosition, type Position, type RoutePlan } from './control';

export type PointInput = { latitude: string; longitude: string; altitude: string };
export type RouteDraft = { points: PointInput[]; speed: string; repeatCount: string; repeatDelay: string };
...
export function readSavedDraft(): RouteDraft | null {
  try {
    ...
    if (typeof draft?.speed === 'string' && Array.isArray(draft.points) && draft.points.length >= 2 && draft.points.length <= 128 &&
      draft.points.every((p: PointInput) => p && ['latitude', 'longitude', 'altitude'].every(key => typeof p[key as keyof PointInput] === 'string'))) return {
...
export function parseDraft(draft: RouteDraft): RoutePlan {
  if (draft.points.length < 2 || draft.points.length > 128) throw new Error('路线须包含 2 至 128 个点');
  const points = draft.points.map((p, index) => {
    try { return parsePosition(p.latitude, p.longitude, p.altitude); }
    catch (e) { throw new Error(`点 ${index + 1}：${e instanceof Error ? e.message : String(e)}`); }
  });
  const speed = Number(draft.speed) / 3.6;
  if (!Number.isFinite(speed) || speed <= 0 || speed > 1000) throw new Error('速度须大于 0，且不超过 3600 km/h');
  const repeat_count = Number(draft.repeatCount);
  const repeat_delay = repeat_count === 1 ? 0 : Number(draft.repeatDelay);
  if (!Number.isInteger(repeat_count) || repeat_count < 1 || repeat_count > 10000) throw new Error('播放次数须为 1 至 10000 的整数');
  if (repeat_count > 1 && (!draft.repeatDelay.trim() || !Number.isFinite(repeat_delay) || repeat_delay < 0 || repeat_delay > 86400)) throw new Error('间隔须为 0 至 86400 秒');
