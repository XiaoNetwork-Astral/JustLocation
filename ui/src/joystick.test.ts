...
it('checks installation and sends a validated numeric speed to the companion', async () => {
  ...
  expect(exec.mock.calls[1][0]).toContain('--es speed 5.4');
  expect(exec.mock.calls[1][0]).toContain('JoystickActivity');
...
it('reports a missing app and rejects Android launch errors even with exit code zero', async () => {
  ...
  const joystick = createJoystick(exec);
  await expect(joystick.check()).rejects.toThrow('安装');
