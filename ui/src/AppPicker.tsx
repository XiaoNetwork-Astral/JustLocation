...
import type { InstalledApp, LoadApps } from './installedApps';
...
export function AppPicker({ selected, onChange, disabled, loadApps }: Props) {
  ...
  useEffect(() => {
    ...
    void loadApps()
      ...
      .catch((error) => {
        if (!cancelled) setError(error instanceof Error ? error.message : '应用列表读取失败');
      })
      .finally(() => {
  ...
  const visible = available
    .filter(
      (app) =>
        (showSystem || !app.isSystem || selected.includes(app.packageName)) &&
  ...
  return (
    <div className="app-picker">
      ...
      <div className="app-list">
        {visible.map((app) => (
          <label className="choice app-choice" key={app.packageName}>
            <input
              type="checkbox"
                ...
                if (!disabled)
                  onChange(
                    event.target.checked
                      ...
                      : selected.filter((name) => name !== app.packageName),
                  );
              }}
            />
            <span>
              ...
              <small>
                {app.packageName}
                {app.isSystem ? ' · 系统应用' : ''}
                {!known.has(app.packageName) ? ' · 未在列表中找到' : ''}
              </small>
            </span>
          </label>
        ))}
      ...
      {!loading && !error && !visible.length && (
        <p className="picker-empty">{query ? '没有找到应用' : '暂无应用，试试刷新列表'}</p>
      )}
    </div>
  );
}
