...
import { parsePosition, convertCoordinates, type CoordinateSystem } from './coordinates';
...
export function PositionEditor({
  ...
  title?: string;
  onClose: () => void;
  onSave: (p: Position, name: string) => Promise<void>;
}) {
    ...
    <>
      ...
      >
        <form
          onSubmit={async (e) => {
            ...
            try {
              const p = read();
              setSaving(true);
              await onSave(p, label.trim() || '自定义位置');
            } catch (error) {
        ...
        >
          ...
          <button
            type="button"
            className="map-entry text-button"
            disabled={saving}
            onClick={openMap}
          >
            <MapPin size={20} />
          ...
          <label className="field">
            坐标系
            <select value={system} onChange={(e) => setSystem(e.target.value as CoordinateSystem)}>
