...
export interface ImportedRoute { name: string; points: Position[] }
...
export function parseGpx(text: string): ImportedRoute[] {
  ...
  if (document.querySelector('parsererror') || root.localName !== 'gpx') throw new Error('无法读取 GPX 文件');
  ...
  function add(element: Element, pointName: string, name: string) {
    ...
    for (const point of children(element, pointName)) {
      ...
      try {
        position = parsePosition(point.getAttribute('lat') ?? '', point.getAttribute('lon') ?? '', children(point, 'ele')[0]?.textContent ?? '0');
      } catch { throw new Error(`${name} 中有无效的坐标或海拔`); }
      const previous = points.at(-1);
      if (!previous || previous.latitude !== position.latitude || previous.longitude !== position.longitude) points.push(position);
    }
    if (points.length < 2) throw new Error(`${name} 至少需要两个不同的路线点`);
    if (points.length > 128) throw new Error(`${name} 有 ${points.length} 个点，目前最多支持 128 个，请先简化轨迹`);
  ...
  for (const element of Array.from(root.children)) {
    ...
    if (element.localName === 'trk') {
      ...
      segments.forEach((segment, i) => add(segment, 'trkpt', segments.length > 1 ? `${name} · 第 ${i + 1} 段` : name));
