...
const view = vi.hoisted(() => ({ markers: [] as { html: string; at: number[] }[], setViews: [] as { at: number[]; zoom: number }[], center: { lat: 31.2, lng: 121.5 } }));
...
vi.mock('leaflet', () => {
  ...
  return {
    ...
    Proj: { CRS: class { constructor(public code: string, public options: unknown) {} } },
...
const { defaultMapProvider, findMapProvider, hasUsableSource, mapProviderKey, resolveTileUrl } = await import('./mapProviders');
const at = { latitude: 31.2, longitude: 121.5, altitude: 12, accuracy: 5, speed: 0, bearing: 0 };

beforeEach(() => { view.markers.length = 0; view.setViews.length = 0; });
...
it('registers the providers with the coordinate system their tiles actually use', () => {
  ...
  // 自定义图源没有地址时不可用，避免出现一张空白地图。
  expect(hasUsableSource(findMapProvider('custom'), '')).toBe(false);
  expect(hasUsableSource(findMapProvider('custom'), 'https://tiles.example/{z}/{x}/{y}.png')).toBe(true);
...
it('centers on a search result so the coordinates come from the selected place', async () => {
  vi.stubGlobal('fetch', vi.fn(async () => ({
    ok: true,
    json: async () => [{ name: '上海火车站', display_name: '上海火车站, 静安区, 上海市', lat: '31.2497', lon: '121.4553' }],
  })));
  render(<MapPicker initial={at} onChoose={vi.fn()} onClose={vi.fn()} />);
  const user = userEvent.setup();
...
it('moves to the current position and reports a denied permission in plain words', async () => {
  ...
  const { unmount } = render(<MapPicker initial={null} onChoose={vi.fn()} onClose={vi.fn()} locate={locate} />);
  const user = userEvent.setup();
  ...
  render(<MapPicker initial={null} onChoose={vi.fn()} onClose={vi.fn()} locate={async () => { throw new Error('没有定位权限，无法定位到当前位置'); }} />);
  await user.click(await screen.findByRole('button', { name: '定位到当前位置' }));
...
it('converts stored WGS84 points before handing them to a GCJ-02 map', async () => {
  render(<MapPicker route={{ points: [at, { ...at, longitude: 121.51 }], onConfirm: vi.fn() }} onClose={vi.fn()} />);
  const user = userEvent.setup();
  await user.selectOptions(await screen.findByLabelText('地图图源'), 'amap');
  // 换图源会重建图层，所以只看最后一次绘制出来的两个点。
  ...
  // GCJ-02 在上海一带会有几百米的偏移，所以红点绝不能等于原始 WGS84 值。
