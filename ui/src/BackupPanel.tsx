...
export function BackupPanel({
  ...
  async function download() {
    ...
    setNote('');
    try {
      setNote(await save(createBackup(localStorage), 'json'));
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    } finally {
      setBusy(false);
    ...
    <section className="settings-card backup-panel">
      ...
      {preview && (
        <div className="import-preview">
          <strong>
            {preview.places.length} 个位置 · {preview.routes.length} 条路线
          </strong>
          ...
          <div className="route-actions">
            <button
              ...
              onClick={() => {
                try {
                  ...
                  setNote(`已添加 ${count.places} 个位置、${count.routes} 条路线`);
                  setError('');
                  setPreview(null);
                  onImported();
                } catch (e) {
            ...
            <button className="text-button" disabled={busy} onClick={() => setPreview(null)}>
              取消
            </button>
          </div>
        </div>
