...
it('starts location and opens the joystick from one switch, and closing it keeps the simulation', async () => {
  ...
  const client = vi
    .fn()
    .mockResolvedValueOnce({ requested_active: false, config })
...
it('keeps the application scope and sends it on the next start', async () => {
  ...
  render(
    <App
      client={client}
      loadApps={async () => [
        { packageName: 'example.selected', appLabel: '地图', isSystem: false },
      ]}
    />,
  );
  await screen.findByText('后台已连接');
...
it('switches the scope back to all apps from the scope page without losing the selection', async () => {
  ...
  expect(JSON.parse(localStorage.getItem('justlocation.scope')!)).toEqual({
    mode: 'all',
    packages: ['example.selected'],
  });
...
it('lets an empty scope open the picker again instead of trapping the user', async () => {
  const client = vi
    .fn()
    .mockResolvedValue({ requested_active: false, config: { position, scope: { mode: 'all' } } });
  render(
    <App
      client={client}
      loadApps={async () => [{ packageName: 'example.one', appLabel: '一个应用', isSystem: false }]}
    />,
  );
  await screen.findByText('后台已连接');
  const user = userEvent.setup();
  ...
  expect(await screen.findByRole('checkbox', { name: /一个应用/ })).toHaveProperty(
    'checked',
    false,
  );
  await user.click(screen.getByRole('button', { name: '完成' }));
...
it('submits the checked apps as the start scope and blocks an empty selection', async () => {
  ...
  expect(client).toHaveBeenLastCalledWith({
    op: 'start',
    config: { position, scope: { mode: 'apps', packages: ['example.new'] } },
  });
});
...
it('renames a saved place without moving the simulation', async () => {
  ...
  localStorage.setItem(
    'justlocation.places',
    JSON.stringify([{ id: 'p1', name: '公司', position, pinned: false }]),
  );
  render(<App client={client} />);
  await screen.findByText('后台已连接');
  const user = userEvent.setup();
...
it('starts and stops a route from the route manager', async () => {
  ...
  const client = vi
    .fn()
    .mockResolvedValueOnce({ requested_active: false, config: { position, scope } })
...
it('reports which system channels are ready instead of hiding them', async () => {
  const client = vi.fn().mockResolvedValue({
    ...
    gnss_hook_ready: true,
    nmea_hook_ready: false,
    cell_query_hook_ready: false,
    cell_callback_hook_ready: false,
...
it('reads a shared map link and lets the coordinate system be corrected', async () => {
  ...
  await user.click(screen.getByRole('button', { name: '导入位置' }));
  await user.type(
    screen.getByLabelText('粘贴地图链接或经纬度'),
    'https://uri.amap.com/marker?position=116.404,39.915',
  );
  ...
  expect(saved.position.latitude).toBeGreaterThan(39.9);
});
...
it('writes the satellite switches straight to the backend from their own panel', async () => {
  const client = vi.fn().mockResolvedValue({
    requested_active: false,
    config: null,
    gnss: { gnss_enabled: false, nmea_enabled: true },
    gnss_hook_ready: true,
    nmea_hook_ready: false,
  });
  render(<App client={client} />);
  ...
  expect(client).toHaveBeenLastCalledWith({
    op: 'set_gnss',
    config: { gnss_enabled: true, nmea_enabled: true },
  });
...
it('manages saved Wi-Fi networks, writes them to the backend, and points to channel readiness', async () => {
  const client = vi.fn().mockResolvedValue({ requested_active: false, config: null });
  render(<App client={client} />);
  await screen.findByText('后台已连接');
  const user = userEvent.setup();
  ...
  expect(screen.getByText(/连接信息与扫描结果是否就绪/)).toBeTruthy();
  ...
  const saved = client.mock.calls
    .map((call) => call[0])
    .filter((op) => op?.op === 'set_wifi')
    .at(-1);
  expect(saved.config.targets).toHaveLength(1);
  expect(saved.config.targets[0]).toMatchObject({
    ssid: '家里',
  ...
  expect(client.mock.calls.every((call) => ['status', 'set_wifi'].includes(call[0]?.op))).toBe(
    true,
  );
});
...
it('keeps the backend status fresh while idle instead of freezing at mount', async () => {
  ...
  try {
    const client = vi
      .fn()
      .mockResolvedValue({ requested_active: false, config: null, hook_connected: false });
    render(<App client={client} />);
...
it('shows a connection failure and never presents an active session', async () => {
  render(
    <App
      client={async () => {
        throw new Error('后台未启动');
      }}
    />,
  );
  expect(await screen.findByRole('alert')).toHaveProperty(
    'textContent',
    expect.stringContaining('后台未启动'),
  );
  expect(screen.getByRole('button', { name: '开始模拟' })).toHaveProperty('disabled', true);
});
...
it('opens the cell data page from the cell switch when no operator is configured', async () => {
  ...
  expect(
    await screen.findByText(
      '还没有查询结果。上面的按钮会按目标位置取这一带的基站，也可以直接导入离线数据。',
    ),
  ).toBeTruthy();
...
it('toggles configured cell simulation from the cell panel without stopping location', async () => {
  const telephony = {
    ...
    radius_m: 700,
    subscriptions: [
      { id: 7, slot: 0, mcc: '460', mnc: '001', country: 'cn', carrier: 'Test', enabled: true },
    ],
  };
  ...
  expect(client).toHaveBeenLastCalledWith({
    op: 'set_telephony',
    config: { ...telephony, cells_enabled: false },
  });
  expect(client).not.toHaveBeenCalledWith({ op: 'stop' });
});
