...
it('exports a portable GPX route with escaped names and exact coordinates and elevation', () => {
  const points = [{ latitude: 0, longitude: 0, altitude: -3, accuracy: 5, speed: 0, bearing: 0 },
    { latitude: 31.23456789, longitude: 121.12345678, altitude: 12.25, accuracy: 5, speed: 0, bearing: 0 }];
  const name = '河边 & <小桥> 🏠';
...
it('reads namespace-qualified tracks in order, including zero coordinates and elevation', () => {
  const routes = parseGpx(`<g:gpx xmlns:g="http://www.topografix.com/GPX/1/1"><g:trk><g:name>河边</g:name><g:trkseg>
  ...
  expect(routes[0].points.map(p => [p.latitude, p.longitude, p.altitude])).toEqual([[0, 0, -3], [0, 0.001, 0]]);
});
...
it('keeps separate segments and routes separate instead of crossing recording gaps', () => {
  ...
  const routes = parseGpx(`<gpx><trk><name>A</name><trkseg>${points}</trkseg><trkseg>${points}</trkseg></trk>
...
it('rejects broken files, missing or invalid coordinates and oversized routes without truncation', () => {
  for (const xml of ['<gpx>', '<html/>', '<gpx><wpt lat="1" lon="2"/></gpx>',
  ...
  expect(() => parseGpx(`<gpx><rte>${Array.from({ length: 129 }, (_, i) => `<rtept lat="1" lon="${i / 1000}"/>`).join('')}</rte></gpx>`)).toThrow('128');
});
