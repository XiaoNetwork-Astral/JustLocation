...
import type { RoutePlan } from './protocol';
...
function readRoutes(): SavedRoute[] {
  try {
    ...
    return entries.flatMap((entry) => {
      try {
        if (typeof entry?.id !== 'string' || typeof entry.name !== 'string' || !entry.name.trim())
          return [];
...
export function RouteLibrary({
  ...
  disabled: boolean;
  getPlan: () => RoutePlan;
  onUse: (plan: RoutePlan) => void;
  onEdit?: (plan: RoutePlan, name: string) => void;
}) {
  ...
  function write(next: SavedRoute[]) {
    try {
      ...
      throw new Error('路线保存失败，请检查面板的存储空间');
    }
    setRoutes(next);
    setError('');
  }
    ...
    <div className="route-library">
          ...
          <div className="saved-routes">
            {routes.map((route, index) => (
              <div className="saved-route" key={route.id}>
                <button
                  className="saved-route-use"
                  aria-label={`使用${route.name}`}
                  disabled={disabled}
                  onClick={() => {
                    onUse(route.plan);
                    setError('');
                ...
                >
                  ...
                  <span>
                    {route.plan.points.length} 个点 · {Number((route.plan.speed * 3.6).toFixed(2))}{' '}
                    km/h · {route.plan.repeat_count ?? 1} 次
                  </span>
                </button>
                <button
                  className="icon-button"
                  aria-label={`导出${route.name}为 GPX`}
                  title="导出 GPX"
                  disabled={exporting}
                  onClick={async () => {
                    setExporting(true);
                    ...
                    setNote('');
                    try {
                      setNote(await saveFile(exportGpx(route.name, route.plan), 'gpx'));
                    } catch (e) {
                      setError(e instanceof Error ? e.message : String(e));
                    } finally {
                      setExporting(false);
                ...
                {onEdit && (
                  <button
                    ...
                    title="编辑路线点"
                    disabled={disabled}
                    onClick={() => onEdit(route.plan, route.name)}
                  >
                    <Pencil size={18} />
                  </button>
                )}
                <button
                  ...
                  onClick={() => {
                    try {
                      write(routes.filter((item) => item.id !== route.id));
                      setDeleted({ route, index });
                    } catch (e) {
                      setError(e instanceof Error ? e.message : String(e));
                    }
                  }}
      ...
      {deleted && (
        <div className="route-undo" role="status">
          <span>已删除“{deleted.route.name}”</span>
          <button
            className="text-button"
            disabled={disabled}
            onClick={() => {
              try {
                const next = [...routes];
                next.splice(deleted.index, 0, deleted.route);
                write(next);
                setDeleted(null);
                setOpen(true);
              } catch (e) {
