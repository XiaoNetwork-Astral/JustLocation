...
it('writes the master switch straight to the backend instead of waiting for a save', async () => {
  ...
  render(<CellPanel target={{ latitude: 1, longitude: 2 }} client={client}
    state={{ requested_active: false, config: null, telephony } as never} onConfigure={configure} onClose={() => {}} />);
  const user = userEvent.setup();
  const master = await screen.findByRole('checkbox', { name: '启用基站模拟' });
  expect(master).toHaveProperty('checked', false);
  await user.click(master);
  await waitFor(() => expect(configure).toHaveBeenCalledWith({ ...telephony, cells_enabled: true }));
