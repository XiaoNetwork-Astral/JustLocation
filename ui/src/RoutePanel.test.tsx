...
const props = (command = vi.fn().mockResolvedValue(undefined)) => ({
  state: idle as never,
  busy: false,
  scope: { mode: 'all' as const } as never,
  onCommand: command,
  onScope: () => {},
});
  ...
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      points: [
        ...
        { latitude: '0', longitude: '0.001', altitude: '0' },
      ],
    }),
  );
...
it('keeps repeat settings in the draft, validates them, and includes them when starting', async () => {
  ...
  expect(command).toHaveBeenCalledWith(
    expect.objectContaining({
      route: expect.objectContaining({ repeat_count: 3, repeat_delay: 10 }),
    }),
  );
  command.mockClear();
  await user.clear(screen.getByLabelText('播放次数'));
      ...
      state={
        {
          ...
          route: {
            plan: {
              points: [position, { ...position, longitude: 1 }],
              speed: 10,
              repeat_count: 3,
              repeat_delay: 10,
            },
...
it('previews an import, replaces the draft only on selection and starts with the existing speed and scope', async () => {
  ...
  await user.upload(
    screen.getByLabelText('导入 GPX'),
    new File(['<gpx>'], 'broken.gpx', { type: 'application/gpx+xml' }),
  );
...
it('saves a named route, reopens it for editing, and restores points, speed and repeats', async () => {
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      repeatDelay: '10',
      points: [
        { latitude: '31', longitude: '121', altitude: '5' },
        { latitude: '31.01', longitude: '121', altitude: '9' },
      ],
    }),
  );
  const command = vi.fn();
