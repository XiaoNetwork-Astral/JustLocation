...
/** Only the slot, subscription ID and public operator fields needed by the settings panel. */
final class PhoneSubscriptions {
    static byte[] encode(List<SubscriptionInfo> records) throws Exception {
        ...
        for (SubscriptionInfo sub : slots.values()) {
            String country = sub.getCountryIso() == null ? "" : sub.getCountryIso().toLowerCase(Locale.ROOT);
            String carrier = sub.getCarrierName() == null ? "" : sub.getCarrierName().toString();
            carrier = carrier.codePoints().filter(c -> !Character.isISOControl(c)).limit(32)
                .collect(StringBuilder::new, StringBuilder::appendCodePoint, StringBuilder::append).toString();
            result.put(new JSONObject().put("id", sub.getSubscriptionId()).put("slot", sub.getSimSlotIndex())
                .put("mcc", digits(sub.getMccString(), "[0-9]{3}"))
                .put("mnc", digits(sub.getMncString(), "[0-9]{2,3}"))
                .put("country", country.matches("[a-z]{2}") ? country : "").put("carrier", carrier));
        }
        return result.toString().getBytes(StandardCharsets.UTF_8);
    }
    private static String digits(String value, String pattern) { return value != null && value.matches(pattern) ? value : ""; }
}
