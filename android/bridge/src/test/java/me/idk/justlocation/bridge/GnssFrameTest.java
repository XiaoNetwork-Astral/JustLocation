...
public class GnssFrameTest {
    @Test public void sentencesShareUtcPositionSpeedAndSatelliteSet() {
        ...
        try {
            ...
            assertTrue(lines.get(0).startsWith("$GPGGA,235959.125,3130.00000,S,12115.00000,W,1,08,"));
            assertTrue(lines.get(1).contains(",A,3130.00000,S,12115.00000,W,19.438,90.000,100926,"));
            assertEquals(8, frame.satellites().size());
            assertEquals(2, lines.stream().filter(line -> line.startsWith("$GPGSV")).count());
            assertTrue(lines.stream().filter(line -> line.startsWith("$GPGSA")).findFirst().get()
    ...
    @Test public void roundedMinutesCarryIntoDegreesAndMidnightChangesTheDate() {
