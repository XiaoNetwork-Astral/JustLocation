...
void* hook(void* target, void* replacement) {
    ...
    if (hook_stubs.contains(target)) return nullptr;
    void* original = nullptr;
    void* stub = shadowhook_hook_func_addr(target, replacement, &original);
    __android_log_print(ANDROID_LOG_INFO, "JustLocation", "ART inline hook end: %p, error=%d", target, shadowhook_get_errno());
...
bool unhook(void* target) {
    ...
    if (found == hook_stubs.end() || shadowhook_unhook(found->second) != 0) return false;
...
extern "C" __attribute__((visibility("default"))) bool justlocation_init_hooks(JNIEnv* env) {
    ...
    if (error != 0) {
        __android_log_print(ANDROID_LOG_ERROR, "JustLocation", "ShadowHook initialization failed: %d", error);
        return false;
    }
    ...
    __android_log_print(ANDROID_LOG_INFO, "JustLocation", "ART symbols open complete; LSPlant init begin");
    lsplant::InitInfo info{
        .inline_hooker = hook,
        .inline_unhooker = unhook,
        .art_symbol_resolver = [](std::string_view name) {
            __android_log_print(ANDROID_LOG_DEBUG, "JustLocation", "ART resolve: %.*s", static_cast<int>(name.size()), name.data());
            // ShadowHook's resolver also handles the compressed debug symbol table.
            void* address = art_handle ? shadowhook_dlsym(art_handle, std::string(name).c_str()) : nullptr;
            return address ? address : symbols.find(name, false);
        },
        .art_symbol_prefix_resolver = [](std::string_view name) { return symbols.find(name, true); },
