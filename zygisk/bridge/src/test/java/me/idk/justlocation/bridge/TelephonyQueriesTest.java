...
public class TelephonyQueriesTest {
    ...
    static class Fixture {
        ...
        final TelephonyQueries.Output output = new TelephonyQueries.Output() {
            public List<?> cells(int subId) {
                return List.of("synthetic:" + subId);
            }
            public Object identity(int subId) {
                return "identity:" + subId;
            }
        };
        Fixture() throws Exception {
            new TelephonyQueries(Service.class, Phone.class, WorkSource.class, Callback.class,
                    pkg -> active && "selected".equals(pkg) ? output : null)
    ...
    @Test
    public void selectedQueriesKeepPermissionChecksAndLeaveOtherCallersAlone() throws Throwable {
        Fixture f = new Fixture();
        assertEquals(List.of("synthetic:" + Integer.MAX_VALUE),
        ...
        assertThrows(SecurityException.class,
                () -> f.service.invoke("getAllCellInfo", "selected", null));
        f.service.hardDenied = false;
        f.service.permitted = true;
        assertEquals(List.of("real"), f.service.invoke("getCachedCellInfo"));
    }
    @Test
    public void asyncResultIsDeliveredOnceOnlyAfterPermissionAndSubscriptionChecks()
            throws Throwable {
        Fixture f = new Fixture();
        ...
        assertEquals(List.of(List.of("synthetic:7")), results);
        assertEquals(0, f.service.radioQueries);
        assertThrows(IllegalArgumentException.class,
                ()
                        -> f.service.invoke("requestCellInfoUpdateInternal", 8, callback,
                                "selected", null, null));
        f.service.permitted = false;
        f.service.invoke("requestCellInfoUpdateInternal", 7, callback, "selected", null, null);
    ...
    @Test
    public void stoppedOrExpiredSnapshotRestoresQueriesAndDoesNotLeakCallerContext()
            throws Throwable {
        Fixture f = new Fixture();
        f.service.invoke("getAllCellInfo", "selected", null);
        f.active = false;
        ...
        var results = new java.util.ArrayList<List<?>>();
        f.service.invoke("requestCellInfoUpdateInternal", 7, (Callback) results::add, "selected",
                null, null);
        assertEquals(List.of(List.of("real")), results);
        f.active = true;
        assertEquals(
                "real identity", f.service.invoke("sendRequest", 62, null, 7, null, null, -1L));
    }
    @Test
    public void partialInstallationStaysInactive() throws Throwable {
        Fixture f = new Fixture();
        f.service.hooks.clear();
        assertThrows(IllegalStateException.class,
                ()
                        -> new TelephonyQueries(Service.class, Phone.class, WorkSource.class,
                                Callback.class, pkg -> f.output)
                                .install((target, around) -> {
                                    if (target.getName().equals("getCellLocation"))
