...
public class TelephonyRegistryAdapterTest {
    public static class Events {
        public static final int EVENT_CELL_INFO_CHANGED = 1, EVENT_CELL_LOCATION_CHANGED = 2,
                                EVENT_SIGNAL_STRENGTHS_CHANGED = 3,
                                EVENT_SIGNAL_STRENGTH_CHANGED = 4, EVENT_SERVICE_STATE_CHANGED = 5;
    }
    ...
    public static class Sink implements Listener {
        ...
        public void onCellInfoChanged(List<?> cells) {
            values.add(cells);
        }
        public void onCellLocationChanged(Object identity) {
            values.add(identity);
        }
        public void onSignalStrengthsChanged(Object signal) {
            values.add(signal);
        }
        ...
        public void onCallStateChanged(int state) {
            values.add("call:" + state);
        }
        public void onServiceStateChanged(State state) {
            values.add(state);
        }
    }
    public static class Record {
        ...
        int subId = 7, phoneId = 0;
        Set<Integer> events = Set.of(1, 2, 3, 4);
        boolean permitted = true, coarsePermitted = true, activeUser = true;
    ...
    public static class State {
        ...
        State(String cell, String operator) {
            this.cell = cell;
            this.operator = operator;
        }
        public State createLocationInfoSanitizedCopy(boolean coarse) {
            return new State(null, coarse ? null : operator);
        }
    }
    public static class Registry {
        ...
        int getPhoneIdFromSubId(int subId) {
            return 0;
        }
        boolean validateEventAndUserLocked(Record record, int event) {
            return record.activeUser && record.events.contains(event);
        }
        boolean checkCoarseLocationAccess(Record record, int sdk) {
            assertTrue(sdk == 1 || sdk == 29);
            return sdk == 1 ? record.permitted : record.coarsePermitted;
        }
        boolean checkFineLocationAccess(Record record, int sdk) {
            assertEquals(29, sdk);
    ...
    static class Fixture {
            ...
            adapter = new TelephonyRegistryAdapter(Registry.class, Record.class, Listener.class,
                    Events.class, (pkg, sub, slot, name, authorized) -> {
                        ...
                        if (name.equals("onServiceStateChanged")) {
                            ...
                            return new State(state.cell == null ? null : "synthetic cell",
                                    state.operator == null ? null : "synthetic operator");
                        }
                        return name.equals("onCellInfoChanged")
                                ? List.of("synthetic:" + sub + ":" + slot)
                                : name.equals("onSignalStrengthChanged") ? 12
                                                                         : "synthetic";
                    });
        ...
        Record add(String pkg, Sink sink) throws Exception {
            ...
            record.callingPackage = pkg;
            record.callback = (Listener) adapter.wrap(registry, pkg, 7, sink);
            registry.mRecords.add(record);
            return record;
        }
    }
    @Test
    public void periodicAndRealCallbacksAgreeWithoutChangingSharedState() throws Exception {
        Fixture f = new Fixture();
        Sink selected = new Sink(), other = new Sink();
        Record r = f.add("selected", selected);
        f.add("other", other);
        ...
        r.callback.onCallStateChanged(2);
        assertEquals("call:2", selected.values.get(4));
    }
    @Test
    public void permissionsUserAndEventsAreRecheckedBeforePeriodicDelivery() throws Exception {
        Fixture f = new Fixture();
        Sink sink = new Sink();
        Record r = f.add("selected", sink);
        r.events = Set.of(1, 2);
        r.permitted = false;
        f.adapter.dispatch();
    ...
    @Test
    public void subscriptionAndSlotFollowRegistrationWhileDefaultSubscriptionResolvesCurrentSlot()
            throws Exception {
        Fixture f = new Fixture();
        Sink sink = new Sink();
        Record r = f.add("selected", sink);
        ...
        r.subId = 9;
        f.adapter.dispatch();
        assertEquals(List.of(List.of("synthetic:9:1")), sink.values);
    }
    @Test
    public void serviceStateUsesRedactedCopyForPeriodicDeliveryAndRestoration() throws Exception {
        Fixture f = new Fixture();
        Sink sink = new Sink();
        Record r = f.add("selected", sink);
        r.events = Set.of(5);
        f.adapter.dispatch();
        State full = (State) sink.values.remove(0);
        assertEquals("synthetic cell", full.cell);
        assertEquals("synthetic operator", full.operator);
        r.permitted = false;
        f.adapter.dispatch();
        State coarse = (State) sink.values.remove(0);
        assertNull(coarse.cell);
        assertEquals("synthetic operator", coarse.operator);
        r.coarsePermitted = false;
        f.adapter.dispatch();
        State hidden = (State) sink.values.remove(0);
        assertNull(hidden.cell);
        assertNull(hidden.operator);
        r.callback.onServiceStateChanged(new State(null, null));
        State event = (State) sink.values.remove(0);
        assertNull(event.cell);
        assertNull(event.operator);
        f.active = false;
        f.adapter.dispatch();
        f.adapter.dispatch();
        assertEquals(1, sink.values.size());
        State restored = (State) sink.values.get(0);
        assertNull(restored.cell);
        assertNull(restored.operator);
