...
/** A synthetic GPS L1 constellation shared by status and NMEA, not RF measurements. */
final class GnssFrame {
    record Satellite(int id, float cn0, float elevation, float azimuth) {}
    private static final List<Satellite> SATELLITES = List.of(
            new Satellite(3, 38, 24, 35), new Satellite(7, 42, 61, 85),
    ...
    private static final DateTimeFormatter TIME = DateTimeFormatter.ofPattern("HHmmss.SSS", Locale.ROOT).withZone(ZoneOffset.UTC);
    private static final DateTimeFormatter DATE = DateTimeFormatter.ofPattern("ddMMyy", Locale.ROOT).withZone(ZoneOffset.UTC);
    final long timestampMs;
    private final double latitude, longitude, altitude, speed, bearing;

    GnssFrame(double latitude, double longitude, double altitude, double speed, double bearing, long timestampMs) {
        if (!Double.isFinite(latitude) || Math.abs(latitude) > 90 || !Double.isFinite(longitude) || Math.abs(longitude) > 180
                || !Double.isFinite(altitude) || !Double.isFinite(speed) || speed < 0
                || !Double.isFinite(bearing) || bearing < 0 || bearing >= 360) {
        ...
        this.speed = speed; this.bearing = bearing; this.timestampMs = timestampMs;
    }
    ...
    List<String> nmea() {
        ...
        result.add(sentence(format("GPGGA,%s,%s,1,08,1.0,%.3f,M,0.000,M,,", time, point, altitude)));
        result.add(sentence(format("GPRMC,%s,A,%s,%.3f,%.3f,%s,,,A", time, point, speed / 0.5144444444444445, bearing, DATE.format(instant))));
        ...
        for (int page = 0; page < 2; page++) {
            ...
            for (Satellite satellite : SATELLITES.subList(page * 4, page * 4 + 4)) {
                body.append(format(",%02d,%02.0f,%03.0f,%02.0f", satellite.id, satellite.elevation, satellite.azimuth, satellite.cn0));
            }
            result.add(sentence(body.toString()));
        }
        result.add(sentence(format("GPVTG,%.3f,T,,M,%.3f,N,%.3f,K,A", bearing, speed / 0.5144444444444445, speed * 3.6)));
    ...
    private static String coordinate(double value, int degreeDigits) {
        ...
        return format("%0" + degreeDigits + "d%02d.%05d", degrees, minutes / 100000, minutes % 100000);
    }
