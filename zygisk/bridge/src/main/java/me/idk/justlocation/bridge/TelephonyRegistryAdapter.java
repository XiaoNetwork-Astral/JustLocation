...
/**
 * Replace each listener's output and retain registry permission checks during periodic delivery.
 */
final class TelephonyRegistryAdapter {
    interface Source {
        Object value(String packageName, int subId, int slot, String callback, Object authorized)
                throws Exception;
    }
    private record Event(int id, Method callback, boolean location) {}
    private final Class<?> listenerType;
    private final Field records, callback, packageName, subscription, phoneId, cellInfo, identity,
            signal, serviceState;
    ...
    TelephonyRegistryAdapter(Class<?> registry, Class<?> record, Class<?> listener,
            Class<?> constants, Source source) throws Exception {
        this.source = source;
        listenerType = listener;
        records = field(registry, "mRecords");
        callback = field(record, "callback");
        packageName = field(record, "callingPackage");
        subscription = field(record, "subId");
        phoneId = field(record, "phoneId");
        cellInfo = field(registry, "mCellInfo");
        identity = field(registry, "mCellIdentity");
        signal = field(registry, "mSignalStrength");
        serviceState = field(registry, "mServiceState");
        sanitizeState = serviceState.getType().getComponentType().getMethod(
                "createLocationInfoSanitizedCopy", boolean.class);
    ...
    Object wrap(Object registry, String pkg, int subId, Object listener) {
        return Proxy.newProxyInstance(listenerType.getClassLoader(), new Class<?>[] {listenerType},
                new Listener(registry, pkg, subId, listener));
    }
    synchronized void track(Object registry) {
    ...
    void dispatch() throws Exception {
        List<Object> current;
        synchronized (this) {
            current = List.copyOf(registries);
        }
        for (Object registry : current) {
            ...
            synchronized (registrations) {
                for (Object record : registrations) {
                    ...
                    if (!(handler instanceof Listener listener) || listener.owner() != this)
                        continue;
                    for (Event event : events) {
                        if (!allowed(registry, record, event))
                            continue;
                        listener.update(record);
                        Object authorized = event.callback.getName().equals("onServiceStateChanged")
                                ? authorizedState(registry, record, listener.slot)
                                : null;
                        if (event.callback.getName().equals("onServiceStateChanged")
                                && authorized == null)
                            continue;
                        Object replacement = source.value(listener.pkg, listener.subId,
                                listener.slot, event.callback.getName(), authorized);
                        if (replacement == null && !listener.synthetic.contains(event.id))
                            continue;
                        Object value = replacement != null ? replacement
                                : authorized != null
                                ? authorized
                                : originalValue(registry, listener.slot, event);
                        // Preserve null cell identities (the framework translates them to empty
                        // locations).
                        if (value == null
                                && !event.callback.getName().equals("onCellLocationChanged"))
                            continue;
                        try {
                            event.callback.invoke(listener.delegate, value);
                            if (replacement == null)
                                listener.synthetic.remove(event.id);
                            else
                                listener.synthetic.add(event.id);
                        } catch (InvocationTargetException error) {
                            // Binder death is cleaned up by TelephonyRegistry's existing death
                            // recipient.
                            if (!isRemoteException(error.getCause()))
                                throw error;
                        }
                    }
                }
    ...
    private boolean allowed(Object registry, Object record, Event event) throws Exception {
        if (!event.location)
            return (boolean) match.invoke(record, event.id);
        return (boolean) validate.invoke(registry, record, event.id)
                && (boolean) coarse.invoke(registry, record, 1)
                && (boolean) fine.invoke(registry, record, 29);
    }
    ...
    private Object authorizedState(Object registry, Object record, int slot) throws Exception {
        ...
        if (state == null)
            return null;
        if ((boolean) fine.invoke(registry, record, 29))
            return state;
    ...
    private Object originalValue(Object registry, int slot, Event event) throws Exception {
        ...
        return switch (event.callback.getName()) {
            case "onCellInfoChanged" -> {
                List<?> values = (List<?>) cellInfo.get(registry);
                yield slot < values.size() ? values.get(slot) : null;
            }
            ...
            case "onSignalStrengthChanged" -> {
                ...
                int strength = value == null ? 99 : (int) gsmSignal.invoke(value);
    ...
    private final class Listener implements InvocationHandler {
        ...
        TelephonyRegistryAdapter owner() {
        ...
        public Object invoke(Object proxy, Method called, Object[] args) throws Throwable {
            ...
            for (Event item : events)
                if (item.callback.getName().equals(called.getName())) {
                    event = item;
                    break;
                }
            if (event == null)
                return forward(called, delegate, args);
            List<?> registrations = (List<?>) records.get(registry);
            synchronized (registrations) {
                slot = (int) resolvePhone.invoke(registry, subId);
                for (Object record : registrations)
                    if (callback.get(record) == proxy) {
                        update(record);
                        break;
                    }
                Object replacement = source.value(pkg, subId, slot, called.getName(),
                        called.getName().equals("onServiceStateChanged") ? args[0] : null);
                Object result = forward(
                        called, delegate, replacement == null ? args : new Object[] {replacement});
                if (replacement == null)
                    synthetic.remove(event.id);
                else
                    synthetic.add(event.id);
                return result;
            }
        }
    }

    private void addEvent(Class<?> constants, String name, String callbackName, boolean location)
            throws Exception {
        int id = constants.getField(name).getInt(null);
        for (Method method : listenerType.getMethods())
            if (method.getName().equals(callbackName)) {
                events.add(new Event(id, method, location));
                return;
            }
    ...
    private static boolean isRemoteException(Throwable error) {
        for (Class<?> type = error.getClass(); type != null; type = type.getSuperclass())
            if (type.getName().equals("android.os.RemoteException"))
                return true;
        return false;
    }
    private static Object arrayValue(Object array, int index) {
