...
__attribute__((noinline)) static int destination(int value) { return value + 7; }
...
int main() {
    ...
    if (mprotect(reinterpret_cast<void*>(page), getpagesize(), PROT_READ | PROT_WRITE | PROT_EXEC)) return 2;
    auto target = reinterpret_cast<uintptr_t>(destination);
    memcpy(reinterpret_cast<void*>(address + 16), &target, sizeof(target));
    __builtin___clear_cache(reinterpret_cast<char*>(address), reinterpret_cast<char*>(address + 24));
    if (existing_veneer(34) != 41 || shadowhook_init(SHADOWHOOK_MODE_UNIQUE, true)) return 3;
    ...
    if (!stub) { printf("hook failed: %d\n", shadowhook_get_errno()); return 4; }
    puts("Veneer installed; calling original trampoline");
    if (existing_veneer(34) != 141) return 5;
    if (shadowhook_unhook(stub) || existing_veneer(34) != 41) return 6;
    alarm(0);
