...
extern "C" JNIEXPORT jboolean JNICALL Java_me_idk_justlocation_bridge_RuntimeProbe_bootstrap(
        JNIEnv* env, jclass, jstring directory, jbyteArray dex) {
    ...
    if (libraries < 0)
        return false;
    void* runtime = prepare_shadowhook(libraries)
                            ? load_library(libraries, "libjustlocation_runtime.so")
                            : nullptr;
    close(libraries);
    if (!runtime)
        return false;
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0, pair))
        return false;
    transport_timeout(pair[0]);
    transport_timeout(pair[1]);
...
extern "C" JNIEXPORT jboolean JNICALL Java_me_idk_justlocation_bridge_RuntimeProbe_ready(JNIEnv*,
                                                                                         jclass) {
    for (int attempt = 0; attempt < 5; ++attempt) {
        char request[2];
        if (!receive_all(peer, request, sizeof(request)) || request[0] != 'S')
            return false;
        // No configuration: every production callback continues using original output.
        uint32_t empty = 0;
        if (!send_all(peer, &empty, sizeof(empty)))
            return false;
        if (request[1] == 1)
            return true;
    }
