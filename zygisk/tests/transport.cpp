...
int main() {
    if (getuid() != 0) {
        puts("Root required for disposable SELinux transition probe");
        return 1;
    }
    int control[2];
    ...
    if (child == 0) {
        ...
        if (!label("/proc/thread-self/attr/sockcreate", "u:r:zygote:s0"))
            _exit(10);
        int old[2];
        if (socketpair(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0, old))
            _exit(11);
        if (!label("/proc/thread-self/attr/sockcreate", ""))
            _exit(18);
        ...
        if (setresgid(1000, 1000, 1000) || setresuid(1000, 1000, 1000))
            _exit(13);
        if (!label("/proc/thread-self/attr/current", "u:r:system_server:s0"))
            _exit(14);
        errno = 0;
        if (send_all(old[0], "X", 1) || errno != EACCES)
            _exit(15);
        if (!send_all(endpoint, "J", 1))
            _exit(16);
        char reply = 0;
        if (!receive_all(endpoint, &reply, 1) || reply != 'K')
            _exit(17);
        _exit(0);
    }
    close(control[1]);
    ...
    close(control[0]);
    char request = 0;
    bool exchanged = endpoint >= 0 && receive_all(endpoint, &request, 1) && request == 'J' &&
                     send_all(endpoint, "K", 1);
    if (endpoint >= 0)
        close(endpoint);
    int status = 0;
    ...
    printf("Disposable child status=%d, root transport exchange=%s\n", status,
           exchanged ? "PASS" : "FAIL");
    if (!exchanged || !WIFEXITED(status) || WEXITSTATUS(status) != 0)
        return 4;
