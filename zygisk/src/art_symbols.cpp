...
bool ArtSymbols::open() {
    struct Loaded { uintptr_t bias = 0; std::string path; } loaded;
    dl_iterate_phdr([](dl_phdr_info* info, size_t, void* context) {
        std::string_view path(info->dlpi_name);
        if (!path.ends_with("/libart.so")) return 0;
...
void* ArtSymbols::find(std::string_view name, bool prefix) const {
    ...
    const auto& header = *reinterpret_cast<const Elf64_Ehdr*>(data_);
    auto fits = [this](size_t offset, size_t length) { return offset <= size_ && length <= size_ - offset; };
    if (header.e_shentsize != sizeof(Elf64_Shdr) ||
        !fits(header.e_shoff, static_cast<size_t>(header.e_shnum) * sizeof(Elf64_Shdr))) return nullptr;
    ...
    for (size_t i = 0; i < header.e_shnum; ++i) {
        ...
        if ((section.sh_type != SHT_SYMTAB && section.sh_type != SHT_DYNSYM) ||
            ...
            !fits(section.sh_offset, section.sh_size)) continue;
        const auto& strings = sections[section.sh_link];
        if (!fits(strings.sh_offset, strings.sh_size)) continue;
        ...
        for (size_t n = 0; n < section.sh_size / sizeof(Elf64_Sym); ++n) {
            ...
            if (!symbol.st_value || symbol.st_shndx == SHN_UNDEF || symbol.st_name >= strings.sh_size) continue;
            const char* start = names + symbol.st_name;
            const auto* end = static_cast<const char*>(memchr(start, 0, strings.sh_size - symbol.st_name));
            if (!end) continue;
